...
const history = useHistory();
  ...
  const [Places_OfferState, setPlaces_OfferState] = useState("Active");
  ...
  const [OfferEndDate, setOfferEndDate] = useState("");
    const [Places_PriceBefore, setPlaces_PriceBefore] = useState(parseInt(0));
  const [Places_discountPrice, setPlaces_discountPrice] = useState(parseInt(0));

    const [Places_PriceNow, setPlaces_PriceNow] = useState(parseInt(0));
    
  const [Places_Imageupload, setPlaces_Imageupload] = useState({
    file: window.location.origin + "/img/Noimage.jpg",
  });

    const changepricePriceBefore = (e) => {
        setPlaces_PriceBefore(e);

        setPlaces_PriceNow(parseInt(e) - parseInt(Places_discountPrice));
    }

    const changepricediscountPrice = (e) => {
        setPlaces_discountPrice(e);
        setPlaces_PriceNow( parseInt(Places_PriceBefore) - parseInt(e));
    }
...
//  const [Places_discountPrice, setPlaces_discountPrice] = useState([{read:true,update:true,delete:true,add:true}]);
  ...
  return (
      ...
      <form onSubmit={(e) => UpdateDetailsPlace(e)}>
        <HeaderDashBoard Title="اضافة عرض جديد" />
              ...
              >
                <div style={{ width: "100%" }}>
                  ...
                  <input
                    ...
                    onChange={(e) => changepricePriceBefore(e.target.value)}
                    required
              ...
              >
                <div style={{ width: "100%" }}>
                  ...
                  <input
                    ...
                    onChange={(e) => changepricediscountPrice(e.target.value)}
                    required
