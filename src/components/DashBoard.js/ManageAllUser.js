...
const ManageAllUser = () => {
  ...
  useEffect(() => {
    try {
     Users.getAll(IdUser).then((res) => setdataProvince(res.data.data));
 ...
 setTimeout(async() => {
  ...
  const UpdateState = (e) => {
    ...
    cookies.set("IdUserReservation", e.key)
        cookies.set("titeusername", e.FirstNameUser + " " + e.LastNameUser)
  ...
  return (
    <div style={{background:"#eef8fe",height:"100vh"}}>
      <HeaderDashBoard Title="ادارة جميع المستخدمين" />
