...
const [IdUserReservation, setIdUserReservation] = useState(cookies.get("IdUserReservation"));
  ...
  useEffect(() => {
    try {
     Reservation.GetAllReservationUser(IdUserReservation,IdUser).then((res) =>
        setdataProvince(res.data.data[0].Reservation)
      );
    } catch (error) {}
  }, []);
  ...
  const UpdateStatusActive = (e) => {
    Reservation.UpdateStatusActive(e.IdApp, e.IdAppUser, e.key);
    setTimeout(() => {
      Reservation.GetAllReservationUser(IdUserReservation,IdUser).then((res) =>
        setdataProvince(res.data.data[0].Reservation)
      );
  ...
  const UpdateStatusResCancel = (e) => {
    Reservation.UpdateStatusResCancel(e.IdApp, e.IdAppUser, e.key);
    setTimeout(() => {
      Reservation.GetAllReservationUser(IdUserReservation,IdUser).then((res) =>
        setdataProvince(res.data.data[0].Reservation)
      );
  ...
  const UpdateStatusInActive = (e) => {
    Reservation.UpdateStatusInActive(e.IdApp, e.IdAppUser, e.key);
    setTimeout(() => {
        Reservation.GetAllReservationUser(IdUserReservation,IdUser).then((res) =>
        setdataProvince(res.data.data[0].Reservation)
      );
      ...
      render: (Status, record) => (
        <div style={{ display: "flex" }}>
          {Status === "Active" ? (
            <div
              className="divwsel"
              onClick={() => UpdateStatusInActive(record)}
            >
              <Tag color="red">InActive</Tag>
            </div>
          ) : (
            <div className="divwsel" onClick={() => UpdateStatusActive(record)}>
              <Tag color="green">Active</Tag>
            </div>
          )}
          <div
            className="divwsel"
            onClick={() => UpdateStatusResCancel(record)}
          >
            <Tag color="orange">cancel</Tag>
          </div>
 ...
 const [Reservations_ID, setReservations_ID] = useState("");
  ...
  setPerson_City(e.Person_City)
    setReservations_ID(e.key);
  };
  ...
  return (
    <div style={{ background: "#eef8fe", height: "100vh" }}>
     <HeaderDashBoard Title={`${ titelplace} / الحجوزات`} />
