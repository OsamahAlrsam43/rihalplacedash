...
const ReviewPlaceManage = () => {
  ...
  useEffect(() => {
    try {
          Review.GetAllReviewplace(IdAppPlace).then((res) => setdataProvince(res.data.data[0].Reviews));
  ...
  if (dataProvince.length < 0) {
    ...
    data.push({
     ...
     username: res.username,
       TextReview: res.TextReview,
        Rate: res.Rate,
      UserId:res.UserId,
        ImageUser: res.ImageUser,
    DateReviewAdd:moment(res.DateReviewAdd).format(FORMAT),
  ...
  async function showConfirm(e) {
    ...
    setTimeout(async() => {
      ...
      async  onOk() {
          axios.delete(`/ReviewPlaces/${IdAppPlace}`, {
            data: {
  ...
  const columns = [
  
    {
      ...
      {
      title: "التعليق",
      dataIndex: "TextReview",
      key: "TextReview",
      render: (text) => <div>{text}</div>,
      ...getColumnSearchProps("TextReview"),
    },
     {
      title: "التصنيف",
      dataIndex: "Rate",
    ...
    ,
     {
      title: "تاريخ الاضافة",
      dataIndex: "DateReviewAdd",
      key: "DateReviewAdd",
      render: (text) => <div>{text}</div>,
      ...getColumnSearchProps("DateReviewAdd"),
    },
    ,
     {
      title: "تاريخ التعديل",
  ...
  return (
    <div style={{background:"#eef8fe",height:"100vh"}}>
      <HeaderDashBoard Title={`${ titelplace} / مراجعة المستخدمين `} />
...
export default ReviewPlaceManage;
