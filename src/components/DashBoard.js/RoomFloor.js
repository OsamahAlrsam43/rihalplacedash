...
const [dataRoom, setdataRoom] = useState([]);
  ...
  async function showConfirm(e) {
    ...
    setTimeout(async() => {
      ...
      async  onOk() {
          axios.delete(`/RoomFloorplace/${IdAppPlace}&${IdFloor}`, {
            data: {
              "RoomsId": e
            }
 ...
 setRomms_PersonNumber(e.Romms_PersonNumber);
  ...
  return (
    <div style={{background:"#eef8fe",height:"100vh"}}>
      <HeaderDashBoard Title={`غرف الطابق ${NameFloor}`} />
...
>
 ...
 <div
       ...
       <div style={{ width: "100%",padding:"10px 15px",display:"flex",flexWrap:"wrap" }}>
        ...
        {data.map((res, i) => 
          <div onClick={() => GoToRoomFloor(res.key)} key={i}  className={res.Rooms_State==="محجوزة" ?"roomres":"floorcontainer"   && res.Rooms_State==="مغلقة" ?"roomresclose":"floorcontainer"}>
